	...
	public String persist() {
		...
		try {
			...
			if (tbAREA.getId() != null) {
				...
				area = tbAREA.getDeSCAREA();

				tbAREA.setDeSCAREA(removerAcentos(tbAREA.getDeSCAREA()));
				tbAREA.setDeSCAREA(tbAREA.getDeSCAREA().trim().toUpperCase());

				tbAREAList = tbAREAService.findAllTbAREAEntities();
				for (int i =0; i<tbAREAList.size();i++) {
					tbAREAList.get(i).setDeSCAREA(removerAcentos(tbAREAList.get(i).getDeSCAREA()));
					tbAREAList.get(i).setDeSCAREA((tbAREAList.get(i).getDeSCAREA().trim().toUpperCase()));
					if(tbAREA.getDeSCAREA().equals(tbAREAList.get(i).getDeSCAREA())) {
						duplicado = "S";
						this.tbAREA =null;
						break;
					}
